"""Grid / walk-forward optimizer — golden semantic reference for P4-T05.

Verbatim extraction (sed line-ranges) of the pure algorithm code from the
previous web app's ``app/api/backtest.py`` (that version was removed from the
repository). Kept because EXECUTION_PLAN.md P4-T05 parity checks grid ranking
and walk-forward fold stitching against this behavior.

Deviations from the original (mechanical, web-layer only — no algorithm change):
- ``_load_bars_or_400`` raises ``ValueError`` instead of FastAPI's
  ``HTTPException`` (the FastAPI dependency is gone).
- ``load_bars`` is imported directly; tests monkeypatch
  ``algorithms_reference.load_bars`` exactly like they patched the old module.

Fixtures remain the executable truth; this file is the human-readable spec.
"""
...
def _load_bars_or_400(symbol: str, timeframe: str, start, end) -> list[dict]:
    ...
    if df is None or df.empty:
        raise ValueError(
            f"No data for {symbol} {timeframe} — press Load Data first (or seed sample data)"
        )
    return [
        {
